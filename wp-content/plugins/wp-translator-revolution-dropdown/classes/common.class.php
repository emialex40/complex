...
class SurStudioPluginTranslatorRevolutionDropDownCommon {

	public static function getAdminCssUrl() {
		
		return SURSTUDIO_TRD_CSS . '/admin.css';
		
	}

	public static function getCommonJSUrl() {

		return SURSTUDIO_TRD_JS . '/common.class.js';
	...
	public static function getAdminWelcomePluginUrl() {
		...
		return admin_url('admin.php?page=' . SurStudioPluginTranslatorRevolutionDropDownConfig::getWelcomeHandle());
		
	}
	
	public static function getAdminPluginUrl() {
		
		return admin_url('admin.php?page=' . SurStudioPluginTranslatorRevolutionDropDownConfig::getAdminHandle());
	...
	public static function loadStyle($_file, $_version=null) {
		...
		$location = SURSTUDIO_TRD_CSS . '/' . $file;
	...
	public static function log($_string, $_key) {
		...
		@error_log($_string . "\n" . str_repeat('-', 100) . "\n\n", 3, SURSTUDIO_TRD_CACHE . '/error_' . $key . '.log');
	...
	public static function getVariableSet($_var_base_name, $_variants=array(), $_discard_empty=true, $_method='POST', $_escape_html=true) {
		...
		for ($i=0; $i<count($_variants); $i++) {
			...
			for ($j=0; $j<count($partial); $j++) {
				...
				if (!SurStudioPluginTranslatorRevolutionDropDownValidator::isEmpty($partial[$j]) || !$_discard_empty) {
	...
	public static function chain($primary_field, $parent_field, $sort_field, $rows, $root_id=0, $maxlevel=250000) {
		...
		$chain = new SurStudioPluginTranslatorRevolutionDropDownChain($primary_field, $parent_field, $sort_field, $rows, $root_id, $maxlevel);
		return $chain->get();
	...
	*/
	...
	public static function renderObject($_object, $_options=null, $_htmlencode=false) {
		...
		if ($_options['type'] == 'file')
			$template = SURSTUDIO_TRD_TEMPLATES . $_options['content'];
	...
	protected static function displayHideBlock($_name, $_html, $_state) {
		...
		if ($_state) {
			...
			while ((!SurStudioPluginTranslatorRevolutionDropDownValidator::isEmpty($occurrence_ini)) && (SurStudioPluginTranslatorRevolutionDropDownValidator::isInteger($occurrence_ini)) && (!SurStudioPluginTranslatorRevolutionDropDownValidator::isEmpty($occurrence_end)) && (SurStudioPluginTranslatorRevolutionDropDownValidator::isInteger($occurrence_end))) {
				$positions[] = array (
					$occurrence_ini,
					$occurrence_end
				);
...
class SurStudioPluginTranslatorRevolutionDropDownValidator {
...
class SurStudioPluginTranslatorRevolutionDropDownChain {
...
class SurStudioPluginTranslatorRevolutionDropDownFileHandler {
	...
	public static function write($_file, $_contents) {
		...
		if (!$file) {
			SurStudioPluginTranslatorRevolutionDropDownConfig::setCacheFlag(false);
			return false;
		}
...
class SurStudioPluginTranslatorRevolutionDropDownParseCSV {
